...
class ConfusionMatrix(object):
    ...
    def print_summary_my(self):
        ...
        lines = []
        # computing precision, recall, and f1
        lines.append('{0:<15} {1:<15} {2:<15} {3:<15}'.format('label', 'precision', 'recall', 'F1'))
        for i in range(self.alphabet.size()):
            precision[i] = self.matrix[i,i] / sum(self.matrix[i,:])
            recall[i] = self.matrix[i,i] / sum(self.matrix[:,i])
            if precision[i] + recall[i] != 0:
            ...
            correct += self.matrix[i,i]
            label = self.alphabet.get_label(i)
            lines.append('{0:<15} {1:<15.6f} {2:<15.6f} {3:<15.6f}'.format(label, precision[i], recall[i], f1[i]))
        # overall_accruacy = correct / sum(sum(self.matrix[:,:]))
        # macro_precision, macro_recall, macro_f1 = numpy.mean(precision), numpy.mean(recall), numpy.mean(f1)
        lines.append( '* Overall accuracy rate = %f' %(correct / sum(sum(self.matrix[:,:]))))
        lines.append( '* Macro precision %f \t recall %f\t F1 %f' %\
            (numpy.mean(precision), numpy.mean(recall), numpy.mean(f1)))
        print('\n'.join(lines))
    ...
    def print_out(self):
        ...
        self.print_matrix()
        macro_f1 = self.print_summary_my()
        return macro_f1

    # def print_out_my(self):
    #     """Printing out confusion matrix along with Macro-F1 score"""
    #     self.print_matrix()
    #     lines = []
    #     lines.append('{0:<15} {1:<15} {2:<15} {3:<15}'.format('label', 'precision', 'recall', 'F1'))
    #     overall_accruacy, macro_precision, macro_recall, macro_f1 = self.print_summary_my()
    #     print overall_accruacy, macro_precision, macro_recall, macro_f1
    #     lines.append('* Overall accuracy rate = %f' % overall_accruacy)
    #     lines.append( '* Macro precision %f \t recall %f\t F1 %f' %\

...
from stst.config import DICT_INDEX_TO_LABEL, DICT_LABEL_TO_INDEX
...
class Model(object):
    ...
    def train(self, train_instances, train_file, out_list=None):
        ...
        for label, train_instances in zip(predict_label, train_instances):
            print('{}\t#\t{}'.format(DICT_INDEX_TO_LABEL[label], train_instances.get_instance_string()), file=f_out)
    ...
    def test(self, dev_instances, dev_file):
        ...
        for label, dev_instance in zip(predict_label, dev_instances):
            print('{}\t#\t{}'.format(DICT_INDEX_TO_LABEL[label], dev_instance.get_instance_string()), file=f_out)
    ...
    def load_model_score(self, train_file):
        ...
        y_pred = ['1:{}'.format(DICT_LABEL_TO_INDEX[x.strip().split("\t#\t")[0]]) for x in y_pred]
        return y_pred
    ...
    def cross_validation(self, data_instances, data_file, k_fold=5, shuffle=False):
        ...
        for fold in range(k_fold):
            ...
            ''' Train Classifier '''
            self.classifier.train_model(data_feature_file_train, self.model_file)  # Attention! self.dev_feature_file
        ...
        for label, train_instance in zip(preds, data_instances):
            print('{}\t#\t{}'.format(DICT_INDEX_TO_LABEL[label], train_instance[1].get_instance_string()), file=f_out)
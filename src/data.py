...
def sentence2id_and_pad(in_file, vocab2id, max_len):
    ...
    with open(in_file, 'r') as fr:
        for line in fr:
            ...
            statis_sent_len.append(len(words))
            ...
            for i in range(2, 7):
                if len(t) <= i:
                    ...
                    xs.append([0] * max_len)
                    continue
                words = [w for w in t[i].split() if w not in g_stop_words]
                statis_sent_len.append(len(words))
                words = words[: max_len]
                xs_len.append(len(words))
                xs.append([vocab2id.get(w, 0) for w in words] + [0] * (max_len - len(words)))
            aux_xs.append(xs)
            aux_xs_len.append(xs_len)
            aux_len.append(len(t)-2)
    print(max(statis_sent_len), sum(statis_sent_len)/len(statis_sent_len))
    return np.array(x, dtype=np.int32), np.array(y, dtype=np.int32), np.array(seq_len, dtype=np.float32), \
            np.array(aux_xs, dtype=np.int32), np.array(aux_xs_len, dtype=np.int32), np.array(aux_len, dtype=np.int32)


def batch_iter(x, y, seq_len, batch_size, aux_xs, aux_xs_len, aux_len, shuffle=False):
    assert len(x) == len(y)
    idx = np.arange(len(x))
    if shuffle:
    ...
    for start_idx in range(0, len(x), batch_size):
        ...
        yield x[excerpt], y[excerpt], seq_len[excerpt], aux_xs[excerpt], aux_xs_len[excerpt], aux_len[excerpt]
...
class Data(object):
    def __init__(self, load=False):
        ...
        self.train_aux_xs = None
        self.train_aux_xs_len = None
        self.train_aux_len = None

        self.test_x = None
        self.test_y = None
        self.test_seq_len = None
        self.test_aux_xs = None
        self.test_aux_xs_len = None
        self.test_aux_len = None


        self.dev_x = None
        self.dev_y = None
        self.dev_seq_len = None
        self.dev_aux_xs = None
        self.dev_aux_xs_len = None
        self.dev_aux_len = None
    ...
    def init_data(self):
        ...
        self.train_x, self.train_y, self.train_seq_len, self.train_aux_xs, self.train_aux_xs_len, self.train_aux_len = sentence2id_and_pad(self.train_file, vocab2id, self.max_len)
        self.dev_x, self.dev_y, self.dev_seq_len, self.dev_aux_xs, self.dev_aux_xs_len, self.dev_aux_len = sentence2id_and_pad(self.dev_file, vocab2id, self.max_len)
        self.test_x, self.test_y, self.test_seq_len, self.test_aux_xs, self.test_aux_xs_len, self.test_aux_len = sentence2id_and_pad(
            self.test_file, vocab2id, self.max_len)

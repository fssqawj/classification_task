...
class Model(object):
    ...
    def _build_model(self, params=None):
        ...
        expand_sent = self.simple_sum(origin_sent, expand_sents, expand_mask)
        ...
        # pooled = tf.concat([origin_sent, pool_2, expand_sent], axis=1)
        # rcnn_size *= 3
    ...
    def simple_sum(self, qestion_rep, passage_repres, passage_mask):
    ...
    def predict(self, sess, data, restore=True, test=False):
        ...
        for batch_test_x, batch_test_y, batch_len, batch_xs, batch_xs_len, batch_aux_len in batch_iter(test_x, test_y, test_seq_len, 200, test_aux_xs, test_aux_xs_len, test_aux_len):
            ...
            proba, pred = sess.run([self.y_prob, self.y_p], feed_dict={self.in_x: batch_test_x,
        ...
        try:
            for epoch in range(epoch_size):
                ...
                if best_acc < cur_acc:
                    ...
                    self.outputs(preds)
                    evaluation.Evaluation(config.dev_file, config.dev_predict_file.format(self.params.model_name))
        except Exception:
            print(Exception)



    def outputs(self, preds, test=False):
        model_name = self.params.model_name
        input_file = config.dev_file
        output_file = config.dev_predict_file.format(model_name)
        if test:
            input_file = config.test_file
            output_file = config.test_predict_file.format(model_name)

        dev_data = open(input_file).readlines()
        with open(output_file, 'w') as fw:
            for pred, dev_str in zip(preds, dev_data):
                dev_str = dev_str.strip()
                print('{}\t#\t{}'.format(config.id2category[int(pred)], dev_str), file=fw)

def main(args):
    data = Data(load=False)
    model = Model(data.we, args)
    ...
    # probs, preds = model.predict(sess, data)
    # print(preds)
    # model.outputs(preds)
    # evaluation.Evaluation(config.dev_file, config.dev_predict_file.format(model.params.model_name))

    # probs, preds = model.predict(sess, data, test=True)
    # print(preds)
    # model.outputs(preds, test=True)
    # evaluation.Evaluation(config.test_file, config.test_predict_file.format(model.params.model_name))



if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--model_name", help='given the model name', type=str, required=True)
